...
const logger = factory.getLogger("Components.WhiteCardManager");
...
export default {
  ...
  methods: {
    updateCards: async function() {
      let cards = await gameService.getAllWhiteCards();
    ...
    createCards: async function() {
      ...
      for (let i = 0; i < cards.length; i++) {
        const card = cards[i];
        try {
          await gameService.createWhiteCard(card);
          logger.info(`Inserted new white card: ${card}`);
        } catch (e) {
    ...
    exportCards: async function() {
      logger.info(() => `Export`);
      const content = await gameService.exportWhiteCards();
      const blob = new Blob([content]);
      saveAs(blob, "whitecards.txt");
    },
    deleteCards: async function() {
      logger.info(() => `Delete cards`);
      await gameService.deleteWhiteCards();
      this.updateCards();
    }
  },
  mounted() {
